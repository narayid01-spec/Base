...
local function isEnemy(player)
    ...
    if State.TeamCheck and LocalPlayer.Team and player.Team then
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        if isEnemy(p) then
            ...
            if hum and hum.Health>0 and part then
                ...
                if worldDist<=State.AimMaxDistance and aimVisible(p,part) then
                    local screen,onScreen,z=Camera:WorldToViewportPoint(part.Position)
                    if onScreen and z>0 then
                        local screenDist=(Vector2.new(screen.X,screen.Y)-center).Magnitude
                        if screenDist<=State.AimFOV and screenDist<bestScore then best,bestScore=p,screenDist end
                    end
                end
            end
        end
...
table.insert(Connections, RunService.RenderStepped:Connect(function(dt)
    ...
    for _,obj in ipairs(Gui:GetDescendants()) do
        if obj:IsA("Frame") and obj~=RadarGui then
            if obj:GetAttribute("DMHBaseTransparency") == nil then obj:SetAttribute("DMHBaseTransparency", obj.BackgroundTransparency) end
