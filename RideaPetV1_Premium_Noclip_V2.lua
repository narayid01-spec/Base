...
local function restoreNoclipCollision(c)
 if not c then return end
 for p,oldValue in pairs(savedCollision)do
  if p and p.Parent and p:IsDescendantOf(c) then p.CanCollide=oldValue end
 end
end
local function disconnectNoclipWatchers()
 if noclipDescConn then pcall(function()noclipDescConn:Disconnect()end);noclipDescConn=nil end
 if noclipHeartbeat then pcall(function()noclipHeartbeat:Disconnect()end);noclipHeartbeat=nil end
end
local function applyNoclipPart(p)
 if not S.noclip or not p or not p:IsA('BasePart') then return end
 if noclipChar and not p:IsDescendantOf(noclipChar) then return end
 if savedCollision[p]==nil then savedCollision[p]=p.CanCollide end
 p.CanCollide=false
end
local function setNoclip(v)
 v=v==true
 if S.noclip==v and (not v or noclipChar==character()) then
  if v then
   for _,p in ipairs(character() and character():GetDescendants() or {})do applyNoclipPart(p)end
  end
  return
 end
 disconnectNoclipWatchers()
 if not v then
  restoreNoclipCollision(noclipChar or character())
 ...
 if not c then return end
 noclipChar=c
 for _,p in ipairs(c:GetDescendants())do applyNoclipPart(p)end
 noclipDescConn=c.DescendantAdded:Connect(function(p)applyNoclipPart(p)end)
 noclipHeartbeat=RS.Heartbeat:Connect(function()
  if not S.noclip then return end
  local current=character()
  if current~=noclipChar then
   restoreNoclipCollision(noclipChar)
   disconnectNoclipWatchers()
   table.clear(savedCollision)
   noclipChar=current
   if current then
    for _,p in ipairs(current:GetDescendants())do applyNoclipPart(p)end
    noclipDescConn=current.DescendantAdded:Connect(function(p)applyNoclipPart(p)end)
   end
   return
  end
  if current then
   for p in pairs(savedCollision)do
    if p and p.Parent and p:IsDescendantOf(current) and p.CanCollide then p.CanCollide=false end
   end
  end
end)
end
