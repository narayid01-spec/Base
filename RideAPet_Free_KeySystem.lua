-- RideAPet Free V15 | Key System | V7-style Egg ESP Independent from Travel
...
local KEY_OK=false
local SUPPORT_URL='https://sociabuzz.com/adamadamhdhbb/tribe'
local VALID_KEYS={}
for i=1,100 do VALID_KEYS[string.format('RIDEFREE-%03d',i)]=true end
local function validKey(v)
 return VALID_KEYS[tostring(v or ''):upper():gsub('%s+','')]==true
end
local function showKeySystem(onSuccess)
 local oldKey=PG:FindFirstChild('RideAPetKeySystem');if oldKey then oldKey:Destroy()end
 local K=Instance.new('ScreenGui');K.Name='RideAPetKeySystem';K.ResetOnSpawn=false;K.IgnoreGuiInset=true;K.DisplayOrder=1000;K.Parent=PG
 local shade=Instance.new('Frame');shade.Size=UDim2.fromScale(1,1);shade.BackgroundColor3=Color3.fromRGB(0,0,0);shade.BackgroundTransparency=.28;shade.Parent=K
 local card=Instance.new('Frame');card.AnchorPoint=Vector2.new(.5,.5);card.Position=UDim2.fromScale(.5,.5);card.Size=UDim2.fromOffset(430,285);card.BackgroundColor3=Color3.fromRGB(8,11,18);card.Parent=shade;corner(card,16);stroke(card,.12)
 local head=label(card,'RIDE A PET  •  FREE',UDim2.new(1,-40,0,32));head.Position=UDim2.fromOffset(20,18);head.Font=Enum.Font.GothamBold;head.TextSize=19
 local sub=label(card,'FREE ACCESS  •  KEY REQUIRED',UDim2.new(1,-40,0,22));sub.Position=UDim2.fromOffset(20,51);sub.TextColor3=Color3.fromRGB(100,255,175);sub.TextSize=10
 local box=Instance.new('TextBox');box.Size=UDim2.new(1,-40,0,42);box.Position=UDim2.fromOffset(20,88);box.BackgroundColor3=Color3.fromRGB(15,20,30);box.TextColor3=Color3.new(1,1,1);box.PlaceholderText='Enter your free key';box.ClearTextOnFocus=false;box.Text='';box.Font=Enum.Font.GothamMedium;box.TextSize=13;box.Parent=card;corner(box,9);stroke(box,.55)
 local status=label(card,'Enter a valid key to continue.',UDim2.new(1,-40,0,24));status.Position=UDim2.fromOffset(20,135);status.TextColor3=Color3.fromRGB(170,180,195);status.TextSize=10
 local unlock=button(card,'UNLOCK FREE VERSION',UDim2.new(1,-40,0,38));unlock.Position=UDim2.fromOffset(20,169);unlock.BackgroundColor3=Color3.fromRGB(18,80,55)
 local support=button(card,'GET KEY / SUPPORT',UDim2.new(1,-40,0,34));support.Position=UDim2.fromOffset(20,214);support.BackgroundColor3=Color3.fromRGB(18,22,32)
 local hint=label(card,'100 valid keys • Free version • no Premium branding',UDim2.new(1,-40,0,20));hint.Position=UDim2.fromOffset(20,254);hint.TextColor3=Color3.fromRGB(120,130,145);hint.TextSize=9
 unlock.Activated:Connect(function()
  if validKey(box.Text) then
   KEY_OK=true;status.Text='KEY VALID • ACCESS GRANTED';status.TextColor3=Color3.fromRGB(100,255,175)
   task.wait(.2);K:Destroy();if onSuccess then onSuccess()end
  else
   status.Text='INVALID KEY • TRY AGAIN';status.TextColor3=Color3.fromRGB(255,110,110)
  end
 end)
 support.Activated:Connect(function()
  pcall(function()if setclipboard then setclipboard(SUPPORT_URL)end end)
  pcall(function()SG:SetCore('SendNotification',{Title='Free Key',Text='Support link copied if clipboard is available.',Duration=3})end)
 end)
 box.FocusLost:Connect(function(enter)if enter then unlock:Activate()end end)
 return K
end

-- GUI
local old=PG:FindFirstChild('RideAPet');if old then pcall(function()old:Destroy()end)end
local G=Instance.new('ScreenGui');G.Name='RideAPet';G.ResetOnSpawn=false;G.IgnoreGuiInset=true;G.Parent=PG;G.Enabled=false
...
local title=label(top,'RIDE A PET  •  FREE V15',UDim2.new(1,-180,1,0));title.Position=UDim2.fromOffset(16,0);title.Font=Enum.Font.GothamBold;title.TextSize=16
...
local function show(tab)
 ...
 if tab=='Dashboard' then
  ...
  action('Free Version','Key verified • free access',function()notify('RideAPet Free','Ready')end)
...
show('Dashboard')
showKeySystem(function() G.Enabled=true end)
